...
import { DEFAULT_WINDOW } from '@/lib/constants/defaults'
...
export default function WindowTool() {
  ...
  useFrame(() => {
    ...
    if (intersection) {
      ...
      const snappedX = snapToWallGrid(localPosition.x, wall.length, DEFAULT_WINDOW.width)

      // Calculate window Y position (height above floor)
      const windowY = Math.min(
        PLACEMENT_CONFIG.WINDOW_DEFAULT_HEIGHT,
        wall.height - DEFAULT_WINDOW.height - 0.1
      )
      ...
      // Offset Y position for window height
      globalPos.y = wall.position.y + windowY + DEFAULT_WINDOW.height / 2
    ...
    (event: any) => {
      try {
        if (!previewPosition || !hoveredWall || !isValidPlacement || !building) {
          console.warn('Cannot place window: invalid position or wall')
          return
        }
        ...
        // Create window opening with local coordinates
        const newWindow = {
          id: nanoid(),
        ...
        const validation = validateOpeningSpacing(wall, newWindow)
        if (!validation.valid) {
          console.warn('Cannot place window: overlaps with existing opening')
          return
        }

        // Add window to wall's openings array
        const updatedOpenings = [...wall.openings, newWindow]
        updateElement(wall.id, { openings: updatedOpenings } as any)

        console.log('Window placed successfully:', newWindow.id)
      } catch (error) {
        handleToolError(error as Error, 'window', { action: 'place' })
      }
    },
    ...
    <>
      ...
      {previewPosition && previewNormal && (
        <OpeningPreview
          ...
          dimensions={{ width: DEFAULT_WINDOW.width, height: DEFAULT_WINDOW.height }}
